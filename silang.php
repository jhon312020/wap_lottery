<?php require_once("includes/head.php");
	...
	$arrSilangDetails = mysql_fetch_array(mysql_query("SELECT * FROM lottery_cms_pages WHERE cms_id = '39'"));
								...
								if($betAmount >= $minbetAmount) {
									foreach($crushPositions as $crushPosition) {
										...
										if(in_array($crushPosition, $crushList)) {
											...
											$arrDiscount = mysql_fetch_array(mysql_query("SELECT * FROM lottery_game_setting WHERE g_market_name = '".$market."' and g_type = '".strtoupper($inputName)."' and g_name = '".$crushPosition."'"));
...
<html lang="en">
	...
	<body>
		...
		<form class="form-horizontal" method="post" action="" name="frm_shio">
				...
				<div class="col-xs-8">
					...
					<?php 
						while($rowAllSilang = mysql_fetch_array($ressilang)) {
							echo "Silang ".$rowAllSilang['g_name'].' = '.$rowAllSilang['g_kei'].' x, ';
						}
						echo "<br>&nbsp;&nbsp;";
						while($rowAllHomoSilang = mysql_fetch_array($resHomo)) {	
							echo "Homo ".$rowAllHomoSilang['g_name'].' = '.$rowAllHomoSilang['g_kei'].' x, ';
						}
					?>
				</p>
				</div> 
			...
			<hr />
			<div class="form-group"> 
				<label class="col-xs-4 control-label">Silang</label> 
				<div class="col-xs-8"><input type="text" class="form-control" placeholder="BET Silang" name="tbk[silang]" id="silang" value="" > </div> 
			</div>
			
			<div class="form-group"> 
				<label class="col-xs-4 control-label">Homo</label> 
				<div class="col-xs-8"><input type="text" class="form-control" placeholder="BET Homo" name="tbk[homo]" id="homo" value="" > </div> 
			</div>
			<div class="form-group"> 
				<div class="col-sm-12">
