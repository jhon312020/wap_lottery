...
switch($data['action']) {
	case 'fetchdiscount': {
		//echo "SELECT * FROM lottery_game_setting WHERE g_market_name = '".$data['market']."' and g_type = '".$data['gameType']."' and g_name = '".$data['cPosition']."'"; exit;
		$arrDiscount = mysql_fetch_array(mysql_query("SELECT * FROM lottery_game_setting WHERE g_market_name = '".$data['market']."' and g_type = '".$data['gameType']."' and g_name = '".$data['cPosition']."'"));
