...
class phptextClass
{
	public function phpcaptcha($type, $textColor,$backgroundColor,$imgWidth,$imgHeight,$noiceLines=0,$noiceDots=0,$noiceColor='#162453')
	{	
		/* Settings */
		$text=rand(111,999);
		...
		if(isset($_SESSION)){
			if($type == 1)
			{
				$_SESSION['head_captcha_code'] = $text;/* set random text in session for captcha validation*/
			}
			else if($type == 2)
			{
				$_SESSION['login_captcha_code'] = $text;/* set random text in session for captcha validation*/
			}
			else if($type == 3)
			{
				$_SESSION['register_captcha_code'] = $text;/* set random text in session for captcha validation*/
			}
			else if($type == 4)
			{
				$_SESSION['admin_captcha_code'] = $text;/* set random text in session for captcha validation*/
			}
			else
			{
