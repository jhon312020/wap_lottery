<?php 
	...
	//////////////////////////////////////  C H E C K   I F   A N Y    R E S U L T    E X I S T ///////////////////////////////////
	if(isset($_REQUEST['key']) && $_REQUEST['key'] == "bbcampuranForm") {
		
		$lotteryNumber = $_REQUEST['lotto_no'];
		$betPrice = $_REQUEST['bet_price']*1000;
		$modBetPrice = number_format( $betPrice , 0 , '.' , ',' );
		
		$arrLotoNumber = str_split($lotteryNumber);
		//print_r($arrLotoNumber); exit;
		//$countLoto = count($arrLotoNumber);
		//if($countLoto > 4) {
		//unset($arrLotoNumber['4']);
		//}
		
		$allCombination = getPossibleCombination($arrLotoNumber);
		$resetCombination = array_unique($allCombination, SORT_REGULAR);
		//echo '<pre>';
		//print_r($resetCombination); exit;
		
		$masterCombo = array();
		foreach($resetCombination as $allComb) {
			$newcombArr = getAllCombos($allComb);
			$masterCombo = array_merge($masterCombo, $newcombArr);
			$resetMasterCombo = array_unique($masterCombo, SORT_REGULAR);		
			$sizeArray = getLengthArray($masterCombo);
		...
		if($modTotalPAmount<$availableBalace) {
			...
			for($i=0;$i<$c;$i++) {
				...
				if($_REQUEST['checkedDigit'][$i]!='') {
					...
					//echo $originalDiscountAmount ; exit;
						...
						<div class="information-page-area">
							...
							<div  class="col-md-12 mrg-top-20">
								<div class="game-body">
									...
									<?php if(isset($_REQUEST['key']) && $_REQUEST['key'] == "bbcampuranForm") {?>
										<div class="col-md-12 showFormCombination4d" >
											<form role="form" name="formbolak" id="formbolak" action="" method="POST">
												...
												<div class="table-responsive">
													<table class="table table-bordered comboTable">
														...
														<tbody>
															<?php 
																$i = 0;
																foreach($resetMasterCombo as $masterCom) {
																	//echo '<pre>';
																	//print_r($masterCom); 
																	
																	$sizeArray = strlen($masterCom);
																	if($sizeArray == '7') {
																		$gameType = "4D";
																		} elseif($sizeArray == '5') {
																		$gameType = "3D";
																	...
																	//echo $modPayAmountFinal; exit;
																	$mCombination = str_replace(" ","",$masterCom);
																...
																<tr>
																	...
																	<td>
																		<input type="text" class="form-control input-width checkLoto" name="d1[]" id="d1_<?php echo $i?>" style="width:110px;" readonly value="<?php echo $mCombination;?>">
																	</td>
																	<td><input type="text" class="form-control input-width GameT" name="gametype[]" id="gametype_<?php echo $i?>" readonly style="color:#F00;" value="<?php echo $gameType;?>"></td>
																	<td><input type="text" class="form-control input-width DiscountP" name="dispercentage[]" id="dispercent_<?php echo $i?>" readonly style="color:#F00;" value="<?php echo $discountPercentage;?>%"></td>
																	...
																	<td><input type="text" class="form-control checkPaybleAmount" name="pay_amount[]" id="show_pay_amount_<?php echo $i?>" readonly="readonly" style="width:90px;" value="<?php echo $modPayAmountFinal;?>"></td>
																</tr>
															<?php $i++;}?>
										...
										<div class="col-md-9 rdc-padding">
											<div class="information-page-area" style="padding:25px 15px; min-height:0;">
												<div class="col-md-12 profileForm">
													<h2>Bolak Balik Campuran</h2>
													<form name="frm" id="member-form-update" method="POST" action="" role="form" enctype="multipart/form-data">
														<input type="hidden" name="key" value="bbcampuranForm">
														<div class="form-group">
															<label for="email">Nomor : </label>
															<input type="text" name="lotto_no" id="lotto_no" maxlength="5">
														</div>
														<div class="form-group">
															<label for="email">Bayaran :</label>
